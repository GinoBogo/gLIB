...
class GOptions : public std::map<std::string, std::any> {
   public:
    ...
    struct Section {
        Section(const std::string& _title) { title = _title; };
    ...
    template <typename T>
    void Insert(const std::string& label, const std::any& value);
...
template <typename T>
void GOptions::Insert(const std::string& label, const std::any& value) {
...
template <typename T>
T GOptions::Get(const std::string& label) {
    T type{};
